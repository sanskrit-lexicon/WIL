...
"""
 ...
 Sep 5, 2018.  Revise to L-code is part of output
"""
...
def match_simple(wilkey,step0dict):
 """match returns a pair (reason,mwkey)
    ...
    2018-09-05.  Return a 3rd field mwLarr,  array of L-codes
       with the given mwkey. Empty list if no match
 """
...
def main(filein,filein1,fileout,fileout1):
 ...
 for wilrec in wilrecs:
  ...
  if reason:
   out1 = "<c>%s</c> <wil>%s</wil> <mw>%s</mw>" %(reason,wilkey,mwkey)
   # 09-05-2018. Add L-code for wilkey, and list of L-codes for mwkey
   mwLs = [rec.L for rec in d[mwkey]]
   mwls1 = ','.join(mwLs)
   out2 = '<wilL>%s</wilL> <mwL>%s</mwL>' %(wilL,mwls1)
   out = out1 + ' ' + out2
   f.write("%s\n" % out)
   nout = nout + 1
